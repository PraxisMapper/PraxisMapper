...
namespace PraxisCore.PbfReader
{
    ...
    /// </summary>
    public class FastPbfReader
    {
        //An alternate take on PBFReader. Doesn't lump progress by block, so it SHOULD be faster in the sense that more progress can be made while long-running items are
        //being processed. But can't currently stop/resume progress in a meaningful way and may not be able to use a cached set of blocks very well.
        ...
        //maybe move this to a new file. FastLoader?
        //This is not yet a possible replacement or alternative to ProcessFileV2. 
        //This does not use the block cache (which should be checked to see how much it actually helps), and can't resume 
        //a broken load or do update checks yet. This is possibly a first-load alternative to the old bulk-load setup but needs tested.
        ConcurrentQueue<ICompleteOsmGeo> waitingToConvert = new ConcurrentQueue<ICompleteOsmGeo>();
        ConcurrentQueue<DbTables.Place> waitingToSave = new ConcurrentQueue<DbTables.Place>();
        bool readingComplete = false;
        public void Version3(string filename, long relationId = 0)
        {
            //Boilerplate intro
            Stopwatch swFile = new Stopwatch();
            swFile.Start();
            ...
            for (var block = nextBlockId; block < Bcount; block++)
            {
                ...
                var stringData = blockData.stringtable.s;
                for (int groupId = nextGroup; groupId < blockData.primitivegroup.Count; groupId++)
                {
                    var group = blockData.primitivegroup[groupId];
                    TagsCollection tags;
                    //regardless of which type we're looking at, we want to only send over tag-matched items to the rest of the pool.
                    switch (itemType)
                    {
                        case 3:
                            foreach (var rel in group.relations)
                            {
                                tags = GetTags(stringData, rel.keys, rel.vals);
                                if (TagParser.GetStyleEntry(tags, styleSet).Name == TagParser.defaultStyle.Name)
                                    continue; //Skip this entry.

                                //add thread to threadpool.
                                ThreadPool.QueueUserWorkItem(ProcessRelation, new ProcessData() { itemId = rel.id, tags = tags });
                            }
                            break;
                        case 2:
                            foreach (var way in group.ways)
                            {
                                tags = GetTags(stringData, way.keys, way.vals);
                                if (TagParser.GetStyleEntry(tags, styleSet).Name == TagParser.defaultStyle.Name)
                                    continue; //Skip this entry.

                                //add thread to threadpool.
                                ThreadPool.QueueUserWorkItem(ProcessWay, new ProcessData() { itemId = way.id, tags = tags });
                            }
                            break;
                        case 1:
                            var nodes = GetTaggedNodesFromBlock(blockData);
                            foreach (var n in nodes)
                            {
                                var p = GeometrySupport.ConvertOsmEntryToPlace(n, styleSet);
                                waitingToSave.Enqueue(p);
                            }
                            break;
                    }
                    if (itemType == 1)
                        break; //skip the sub-groups for these nodes, all processed per block.
                }
            }
            readingComplete = true;
            //Main reader is done, has made a thread for every matched OSM item.
            
            //wait for everything else to complete

        }

        public void ConvertQueuedGeoToPlace() //This should be fired off all the time
        {
            if (waitingToConvert.TryDequeue(out var g))
            {
                if (g == null)
                    return;
                ...
                var place = GeometrySupport.ConvertOsmEntryToPlace(g, styleSet);
                if (place != null)
                    waitingToSave.Enqueue(place);
            }
        }

        public void SaveQueuedPlaces()
        {
            //Preps items, saves in batches.
            //No update support yet. This is the batch-loader equivalent for now.
            
            //This needs to loop while any work items are still processing, so i need a count of total items processed and a flag that the main feeder
            //thread is completed.
            while (!readingComplete) //and we haven't processed everything yet. The hard part is catching if there's 0 queued items but a running thread we should wait for.
            {
                int batchSize = 1000;
                int loaded = 0;
                List<DbTables.Place> waiting = new List<DbTables.Place>(batchSize);
                while (loaded < batchSize)
                {
                    if (waitingToSave.TryDequeue(out var place))
                    {
                        waiting.Add(place);
                        loaded++;
                    }
                ...
                //now we have enough to try and load
                var db = new PraxisContext();
                db.ChangeTracker.AutoDetectChangesEnabled = false;
                db.Places.AddRange(waiting);
                db.SaveChanges();
                db.Dispose();
            }
        }
        ...
        public void ProcessRelation(Object stateInfo)
        {
            //we got passed stuff from a thread.
            ProcessData data = (ProcessData)stateInfo;
            var r = MakeCompleteRelation(data.itemId, tags: data.tags);
            var place = GeometrySupport.ConvertOsmEntryToPlace(r, styleSet);
            if (place != null)
                waitingToSave.Enqueue(place);
        }
        public void ProcessWay(Object stateInfo)
        {
            //we got passed stuff from a thread.
            ProcessData data = (ProcessData)stateInfo;
            var w = MakeCompleteWay(data.itemId, tags: data.tags);
            var place = GeometrySupport.ConvertOsmEntryToPlace(w, styleSet);
            if (place != null)
                waitingToSave.Enqueue(place);
        }

        public void ProcessNode(Object stateInfo)
        {
        ...
        private CompleteRelation MakeCompleteRelation(long relationId, bool ignoreUnmatched = false, PrimitiveBlock relationBlock = null, TagsCollection tags = null)
        {
            try
            {
        ...
        private CompleteWay MakeCompleteWay(long wayId, int hint = -1, bool ignoreUnmatched = false, bool skipTags = false, TagsCollection tags = null) //skipTags is for relations, where the way's tags don't matter.
        {
            try
            {
                ...
                return MakeCompleteWay(way, wayBlock.stringtable.s, ignoreUnmatched, skipTags, tags);
            }
            catch (Exception ex)
            {
        ...
        /// <param name="ignoreUnmatched">if true, returns null if this element's tags only match the default style.</param>
        /// <returns>the CompleteWay object requested, or null if skipUntagged or ignoreUnmatched checks skip this elements, or if there is an error processing the way</returns>
        private CompleteWay MakeCompleteWay(Way way, List<byte[]> stringTable, bool ignoreUnmatched = false, bool skipTags = false, TagsCollection tags = null)
        {
            try
            {
                if (tags == null)
