...
namespace Larry
{
    public static class GeometryHelper
    {
        ...
        public static Geometry GetGeometryFromCompleteWays(List<OsmSharp.Complete.CompleteWay> shapeList, OsmSharp.Complete.CompleteRelation r)
        {
            //A common-ish case looks like the outer entries are lines that join together, and inner entries are polygons.
            //Let's see if we can build a polygon (or more, possibly)
            List<Coordinate> possiblePolygon = new List<Coordinate>();
            ...
            //Separate sets
            var innerEntries = r.Members.Where(m => m.Role == "inner").Select(m => m.Member.Id).ToList(); //these are almost always closed polygons.
            var outerEntries = r.Members.Where(m => m.Role == "outer").Select(m => m.Member.Id).ToList();
            ...
            if (existingPols.Count() == 1)
            {
                ...
                foreach (var ir in innerPolys)
                {
                    if (ir.Nodes.First().Id == ir.Nodes.Last().Id)
                    {
                        var innerP = factory.CreateLineString(Converters.CompleteWayToCoordArray(ir));
                        returnData.InteriorRings.Append(innerP);
                    }
                }
                return returnData;
